import { MDXComponentsSet } from "@/components/mdx";
import type {
  TPostFrontmatter,
  TPostListItem,
  TPostTOCItem,
} from "@/types/docs.type";
import { nanoid } from "nanoid";
import { MDXRemote, type MDXRemoteSerializeResult } from "next-mdx-remote";
import Link from "next/link";
import { PostDate } from "../utils/PostDate";

export const PostRender = (props: {
  compiledSource: MDXRemoteSerializeResult;
  tocList: TPostTOCItem[];
  frontMatter: TPostFrontmatter;
  postId: string;
  nextPostListItem: TPostListItem | null;
  prevPostListItem: TPostListItem | null;
}) => {
  const compiledSource = props.compiledSource;
  return (
    <div className="typesetting">
      <div>
        {props.frontMatter.tags && (
          <div className="my-1 flex flex-wrap">
            {props.frontMatter.tags.map((tagName) => (
              <Link
                className="tag-chip not-prose my-1 mr-2"
                href={`/tags/${tagName}`}
                key={`tags-${nanoid()}`}
              >
                {tagName}
              </Link>
            ))}
          </div>
        )}
        <div
          className={
            "caption-font my-2 flex justify-start text-left whitespace-normal break-words font-bold text-[1.65rem] text-black leading-[2.1rem] dark:text-white"
          }
        >
          {props.frontMatter?.title}
        </div>
        {props.frontMatter?.subtitle && (
          <div
            className={
              "my-1 flex justify-center font-bold text-xl content-font"
            }
          >
            {props.frontMatter.subtitle}
          </div>
        )}
        <PostDate
          className="my-1 flex justify-start text-sm italic"
          date={props.frontMatter?.time}
        />
        {props.frontMatter?.summary && (
          <p
            className={
              "mt-3 mb-2 border-gray-400 border-l-4 bg-gray-100 py-4 pr-2 pl-5 text-[16px] text-gray-800 content-font dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300"
            }
          >
            {props.frontMatter?.summary}
          </p>
        )}
      </div>
      <div
        className={`text-wrap border-gray-500 content-font ${!props.frontMatter.allowShare ? "select-none" : ""}`}
        // {...handleLeftSwipe}
      >
        {compiledSource && (
          <MDXRemote
            compiledSource={compiledSource.compiledSource}
            // @ts-ignore
            components={MDXComponentsSet}
            frontmatter={compiledSource.frontmatter}
            scope={compiledSource.scope}
          />
        )}
      </div>
    </div>
  );
};
