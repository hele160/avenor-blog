...
export const DrawerTOC = (props: { data: TPostTOCItem[] }) => {
  ...
  const activeId = useActiveHeading(
    props.data.map((item) => `#${item.anchorId}`),
  );
  return (
    <Sheet onOpenChange={setIsTOCOpen} open={isTOCOpen}>
      <SheetTrigger
        className="fixed right-5 bottom-16 border border-gray-700 bg-[hsl(var(--background))] shadow-xl dark:border-gray-500 dark:bg-black"
      ...
      <SheetContent side={"right"}>
        <SheetHeader>
          <SheetTitle className="mt-8 text-center font-bold text-base">
            {"TABLE OF CONTENTS"}
          </SheetTitle>
        </SheetHeader>
